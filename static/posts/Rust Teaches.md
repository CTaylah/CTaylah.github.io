Lifetimes ensure that references are valid for as long we need them to be. Every reference in Rust has a lifetime that is the scope in which that reference is valid.

Most of the time, the Rust compiler will infer lifetimes for us.
